package lekcja38_wyrażeniaLambda.zad2;
...
public class EmailManager {
    public static void main(String[] args) {
        ...
        List<Email> bartEmails = filter(emailList, email -> email.isSenderOrRecipient("bart@example.com"));
    ...
    private static List<Email> filter(List<Email> emails, Predicate<Email> predicate) {
        List<Email> filtereEmails = new ArrayList<>();
        for (Email email : emails) {
            if (predicate.test(email))
                filtereEmails.add(email);
        }
        return filtereEmails;
    }
